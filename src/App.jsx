...
function App() {
  ...
  const scrapeUrls = useCallback(async (text) => {
    const urlRegex = /(https?:\/\/[^\s]+|www\.[^\s]+)/g;
    let urls = Array.from(text.match(urlRegex) || []);
  ...
  const generate = async () => {
    ...
    try {
      ...
      const scrapeResult = await scrapeUrls(userMessage);
      const currentParts = [
...
<body>
  <img src="${imageUrl}" alt="Generated Image" />
          ...
          currentMergedFiles = [{ path: 'index.html', content: imageHtml }];
          setGeneratedProject({ files: { 'index.html': imageHtml } });
          fullText = 'Image generated successfully.';
          setStreamingRaw(fullText);
        } else {
          ...
          if (text) {
            ...
            if (currentFiles.length > 0) {
              currentFiles.forEach((cf) => {
                ...
                if (idx !== -1) currentMergedFiles[idx] = cf;
                else currentMergedFiles.push(cf);
              });
              ...
              if (latestFile && !filesFromCurrentTurn.includes(latestFile.path)) {
                filesFromCurrentTurn.push(latestFile.path);
              }
            }
          }
        }
      ...
      setChatMessages([{ role: 'user', content: userMessage }, { role: 'assistant', content: fullText }]);
      ...
      if (finalFiles.length > 0) {
        ...
        setGeneratedProject({ files: { 'index.html': fullText } });
      }
  ...
  const sendChatMessage = async () => {
    ...
    try {
      ...
      const scrapeResult = await scrapeUrls(msg);
      const currentParts = [
        { text: scrapeResult.text },
        ...attachedImages.map((img) => ({ inlineData: { data: img.data, mimeType: img.mimeType } })),
        ...scrapeResult.images.map((img) => ({ inlineData: { data: img.data, mimeType: img.mimeType } })),
      ...
      if (selectedModel === 'gemini-2.5-flash-image') {
        ...
        if (imageUrl) {
          currentMergedFiles = [{ path: 'index.html', content: `<!DOCTYPE html>
<html>
<head>
  <title>Generated Image</title>
  <style>
...
<body>
  <img src="${imageUrl}" alt="Generated Image" />
      ...
      const finalMessages = [...baseMessages, { role: 'assistant', content: fullText }];
      ...
      if (finalFiles.length > 0) {
        ...
        merged['index.html'] = fullText;
      }
      setGeneratedProject({ files: merged });
      setGeneratedHTML(fullText);
    } catch (err) {
      setError(err.message);
